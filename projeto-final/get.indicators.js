...
async function fetchData() {
  try {
    ...
    if(input.close.length > input.period) {
      const sma = new ti.SMA(input).getResult();
      const ema = new ti.EMA(input).getResult();
      const wma = new ti.WMA(input).getResult();
      const rsi = new ti.RSI(input).getResult();
      const bollingerBands = new ti.BollingerBands(input).getResult();

...
const checkVolumeAndTrend = (candles, volumeThreshold, trendConfirmationThreshold) => {
  ...
  } else if (lastVolume < volumeThreshold && lastVolume < firstVolume && lastVolume <= trendConfirmationThreshold) {
    console.log(`\n\nBaixo volume (${lastVolume}) - Reversão de tendência de baixa`);
    // Implemente a lógica para reversão de tendência de baixa aqui
  } else {
    console.log('Volume ou tendência inconclusivos');
